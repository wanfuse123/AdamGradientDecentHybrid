...
torch.cuda.set_per_process_memory_fraction(0.9)  # Limit GPU memory usage to 90% (3.2GB out of 3.5GB)

# Set up RAM limit
ram_limit_gb = 20
torch.cuda.empty_cache()
torch.cuda.memory.set_per_process_memory_fraction(min(ram_limit_gb / torch.cuda.get_device_properties(device).total_memory, 1.0))
...
class EncodedDataset(Dataset):
    ...
    def __len__(self):
        return len(self.current_blocks)
    ...
    def __getitem__(self, idx):
        ...
        return current_inputs.input_ids.squeeze(), current_inputs.attention_mask.squeeze(), next_inputs.input_ids.squeeze()

def train_model(current_blocks, next_blocks, model, batch_size, max_length, accumulation_steps=4):
